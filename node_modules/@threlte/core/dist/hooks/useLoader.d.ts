...
declare type LoaderProto = {
    new (): Loader;
};
export declare type UseLoaderLoadInput = string | string[] | Record<string, string>;
declare type LoaderResultType<TLoader extends Loader> = Awaited<ReturnType<TLoader['loadAsync']>>;
export declare type UseLoaderLoadResult<TLoader extends Loader, Input extends UseLoaderLoadInput, ResultType = LoaderResultType<TLoader>> = Input extends string ? AsyncWritable<ResultType> : Input extends string[] ? AsyncWritable<ResultType[]> : AsyncWritable<Record<keyof Input, ResultType>>;
declare type UseLoaderLoadTransform<TLoader extends Loader> = (result: LoaderResultType<TLoader>) => any;
declare type UseLoaderLoadOptions<TLoader extends Loader> = {
    onProgress?: (event: ProgressEvent) => void;
    transform?: UseLoaderLoadTransform<TLoader>;
};
declare type ThrelteUseLoader<TLoader extends Loader> = {
    loader: TLoader;
...
export declare type UseLoaderOptions<TLoader extends Loader> = {
    /**
